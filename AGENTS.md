...
**Phase**: pre-mainnet on Solana (USDC rail still on **devnet**) but **post-cutover on Stripe** — card payments are LIVE on real money as of 15 May 2026. **Corrected 2026-08-10**: mainnet launch of the Solana program is no longer planned as "wait for a clean external audit" — that's not affordable on the current timeline. The actual plan is a **capped** mainnet launch (`capped-mainnet-plan.md`) with the external audit pursued in parallel via Pro-tier revenue once there's a streamer cohort. The Stripe rail launched separately because Sec3 explicitly advised audits should follow PMF rather than precede it — letting fiat flow earlier means streamers can earn while the Solana side works toward a capped launch.
...
- Live **multi-currency Stripe rail**: `src/lib/currency.ts` (fiat config + `stripeMinAmount` floors), `profiles.settlement_currency` mirror, dual-secret webhook, self-healing Connect route, end-early floor → `paymentIntents.cancel()` fallback. See "Common gotchas" + "Payment flow".
- Compliance: `/legal/*` (imprint/privacy/terms, TDS as operator), essential-only cookie notice.
...
- **On-stream growth tag — REMOVED 2026-09-07**: `▸ get on stream · casi.gg` used to render on the `/obs?layer=beams` overlay when a beam is live, but it collided visually with casi-obs's own permanent glowing "casi.gg" mark in the same bottom-left corner (that mark is server-side HTML on casi-obs, `casi-control.service`'s `STANDBY_HTML`, not part of this repo). Removed rather than repositioned, since the two were redundant anyway. Growth/referral plan (still relevant for other channels): [`docs/growth-referral-plan.md`](./docs/growth-referral-plan.md).
- **CasiCut** — stream-to-shorts pipeline, but **no longer part of this app**. As of 2026-07-07 it runs entirely outside casi-app: `record_watcher.py` (systemd on casi-obs) queues finished OBS recordings as local job files; `sync_recordings.py` on Solomon (a separate Windows GPU box) pulls them over SSH; Solomon's `worker.py` (nightly Task Scheduler run) does transcribe → Claude clip selection → ffmpeg render; a local Flask app (`review_server.py`, port 5151 on Solomon) is the review/post/reject UI. Nothing in this repo talks to it — the `/casicut` web page + `/api/casicut/*` routes were removed 2026-07-08 as dead code once the migration made them unreachable. **Correction 2026-08-10**: the `casicut_jobs`/`casicut_clips` tables themselves were NOT dropped, contrary to what this line used to claim — confirmed still present in the live Supabase project (`casicut_jobs`: 6 rows, `casicut_clips`: 33 rows, plus an undocumented `casicut_word_events`: 812 rows) as of this correction. They're just unreferenced by any code in this repo now. Historical data, not active state — but don't assume they're gone if something ever needs to check.
...
- **Builders page** (`/builders`): developer-facing landing page for the `casi-escrow` Solana primitive. Targets Solana developers evaluating the escrow program as a reusable building block. Visually consistent with `/` (v9 chrome) but content-scoped to the on-chain primitive story, not the streamer product. Supports the Solana Foundation grant narrative.

**robots.txt is Cloudflare-managed, not in this repo.** casi.gg fronts Vercel with Cloudflare, which serves a managed robots.txt that **blocks AI crawlers** (GPTBot/ClaudeBot/Google-Extended/CCBot/…) with `ai-train=no`. Adding `app/robots.ts` may be overridden. Changing AI-crawler access is a **Cloudflare dashboard** action (AI Crawl Control), not code — and it currently conflicts with the goal of being cited by AI search.
...
src/app/
  ...
  builders/           developer landing page for the casi-escrow primitive.
                      Targets Solana devs / grant audience, not streamers.
  join/               LEGACY simple signup page (direct supabase.auth.signUp).
                      DO NOT route new users here — /login is the canonical
                      auth surface. /join inserts a bare profiles row without
                      username selection, producing orphan accounts.
  v/                  DEV TEST page — fetches first profile for local beam
                      testing. Not linked in production. Not a user surface.
  api/
    ...
    webhooks/         solana/ (Helius on-chain events → DB)
                      pumpfun/ (PumpFun token event ingestion)
...
- **Cranker balance drift**: the cranker is the silent single point of failure for the delegated approve/kick flow. Monitor its SOL balance; if it hits 0, every approve AND every kick starts popping a wallet prompt again (routes return 503, admin falls back). Steady-state cost is ~10k lamports per start OR settle (base fee × 2 sigs). **Corrected 2026-08-29**: this section used to say a one-time ~0.00204 SOL ATA rent came out of the cranker's own balance the first time a fresh streamer/viewer ATA was created — that was true before the fix in `programs/casi-escrow/src/lib.rs` (`ATA_RENT_LAMPORTS` / `reimburse_ata_rent`, see `docs/fable-security-review-2026-08-10.md` Finding 3). `initialize_escrow` now pre-funds that rent from the *viewer's* own SOL at booking time, and every settle-shaped instruction reimburses whoever processes it (the cranker, in the delegated path) out of that buffer — so a fresh-ATA settle no longer costs the cranker anything beyond the normal ~10k-lamport fee. The daily `cranker-monitor` cron thresholds (0.005 / 0.001 SOL) now have much more headroom than when they were set.
- **`register_streamer` required before a streamer can be booked** (added 2026-08-29, see `docs/fable-security-review-2026-08-10.md` Finding 4 and `programs/casi-escrow/PRIMITIVE.md`): `initialize_escrow` now requires a `StreamerRegistry` account for whatever `streamer` pubkey a viewer targets — a one-time, ahead-of-time opt-in, not a per-booking signature. Wired into the existing "connect Solana wallet" flow in `PayoutsSection.tsx` (one extra signature the moment a streamer links their wallet), with a "Finish setup →" retry card if that signature fails or a wallet was linked before this feature existed. A streamer with `solana_wallet` set but no on-chain registration will have every Solana booking silently fail with `AccountNotInitialized` until they complete it.
...
- **`/builders` and grant copy maintenance**: the Solana Foundation grant narrative and /builders page need staying updated as the program evolves. Fable drafts revisions when the escrow primitives change.
- **Abuse review assist**: when a flagged beam/flash comes in via `/api/abuse/report`, Fable can pre-classify severity and draft the moderator decision rationale.

**What Fable is NOT for here**: don't use it for code generation in this repo — use a code-capable model (Sonnet/Opus). Don't use it to generate Anchor program logic, RLS policies, or payment flow changes.
