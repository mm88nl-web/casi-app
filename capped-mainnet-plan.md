...
| Cap evasion via multi-account abuse | Same cap applies per `profile_id`; multi-accounting requires multiple full signups. **Corrected 2026-08-10 — "rate-limit on signup IP already exists" was unverified and is not enforced by this repo.** `src/app/login/page.tsx` calls `supabase.auth.signUp()` directly from the client with the public anon key; a scripted attacker can call Supabase Auth's REST endpoint directly and skip this codebase's UI (and its Turnstile widget) entirely. The actual enforcement boundary, if any, is the Supabase Dashboard → Authentication → Attack Protection → CAPTCHA-for-signup toggle (GoTrue-side, same class of "must verify in dashboard, can't confirm from code" gap AGENTS.md already documents for OAuth providers) — **verify this is enabled before relying on it as a mitigation**. Fixed in code: the signup form's step-1 submit now also gates on `captchaToken` being set (previously only checked `acceptedTos`), so a blocked/failed Turnstile widget can no longer let a legitimate-path signup silently carry no captcha token through to `signUp()` — see `docs/fable-spam-abuse-review-2026-08-10.md` Finding 4. This closes the accidental-bypass case but not a deliberate direct-API attacker; that requires the dashboard setting. |
