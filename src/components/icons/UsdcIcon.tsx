...
type Props = {
  size?: number;
  /** Force a flat fill instead of the canonical Circle blue — for cases
...
/**
 * USDC Token mark. Sourced from Circle's official brand kit
 * (Logo.zip → usdc/Token Logo/USDC Token.svg). Blue ring (#0B53BF) with
 * inner white "$" + dual arc indicator.
 */
export default function UsdcIcon({ size = 14, mono, className, title = 'USDC' }: Props) {
  const ringFill = mono ?? '#0B53BF';
  const innerFill = mono ? 'currentColor' : '#FFFFFF';
  return (
    <svg
      width={size}
      height={size}
