...
/**
 * POST /api/solana/delegates/approve-flash
 *
 * Server-side crank for `approve_flash_delegated`. Captures a pending Solana
 * flash on-chain using the streamer's session key instead of their wallet,
 * so the admin Approve button doesn't pop a wallet popup.
 *
 * approve_flash_delegated transfers the full escrowed USDC to the streamer
 * and closes the vault + EscrowState. Cranker pays fees + the one-time
 * streamer-ATA rent if it doesn't already exist. Compromised session key
 * can at worst force a capture of a flash that follows the escrow's
 * declared streamer field — funds can't be redirected elsewhere.
 *
 * DB writes happen in the Helius webhook (approve_flash_delegated
 * discriminator routes into the same handler as approve_flash).
 */
...
export async function POST(req: Request) {
  ...
  const { data: delegate, error: delegateErr } = await supabase
    ...
    .maybeSingle();
  if (delegateErr) {
    logError('delegates-approve-flash', delegateErr, { profile_id: user.id });
  ...
  try {
    ...
    logError('delegates-approve-flash', err, { profile_id: user.id, scope: 'decrypt' });
  ...
  if (session.publicKey.toBase58() !== delegate.session_pubkey) {
    logError('delegates-approve-flash',
  ...
  const cranker = loadCrankerKeypair('delegates-approve-flash');
  if (!cranker) {
    logWarn('delegates-approve-flash', 'SOLANA_CRANKER_KEYPAIR not set — delegated approve cannot pay fees');
  ...
  try {
    ...
    const ix = await client.buildApproveFlashDelegatedIx({
      escrowId:   flash.id,
      streamer,
      viewer,
    ...
    logError('delegates-approve-flash', err, {
      flash_id: flash.id,
